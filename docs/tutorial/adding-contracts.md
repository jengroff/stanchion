...
# 3. Decorate the node
@stanchion_node("search", contract)  # (3)!
...
async def main():
    result = await runner.run([search], Query(text="stanchion", max_results=3))
...
3. The `@stanchion_node` decorator attaches the contract to the function so the runner can find it.
