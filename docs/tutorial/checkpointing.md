...
async def main():
    ...
    runner1 = StanchionRunner(registry, checkpoint_mgr, config1)
    ...
    runner2 = StanchionRunner(registry, checkpoint_mgr, config2)
...
```python
from stanchion import RedisStore, CheckpointManager
...
```python
from pydantic import BaseModel
from stanchion import CheckpointStore
...
1. No need to subclass anything — just implement `save`, `load`, and `delete`. Stanchion uses structural typing (Python protocols).
