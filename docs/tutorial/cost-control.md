...
LLM pipelines can get expensive fast. Stanchion tracks three resource dimensions and lets you set hard limits on any of them.
...
```python
from stanchion import ExecutionBudget, StanchionRunner
...
!!! tip
    ...
    ```python
    from stanchion import FailureClass
    ...
    runner = StanchionRunner.quick(classifiers=[budget_is_terminal])
    ```
