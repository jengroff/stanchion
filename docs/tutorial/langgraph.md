...
If you already use [LangGraph](https://github.com/langchain-ai/langgraph), Stanchion can add contract validation to your existing graphs without rewriting them.
...
Use `@stanchion_langgraph_node` to attach contracts to your LangGraph node functions:
...
from stanchion.adapters.langgraph import (
    LangGraphAdapter,
    stanchion_langgraph_node,
)
...
@stanchion_langgraph_node("add_one", contract)  # (1)!
async def add_one(state: InputState) -> dict:
    return {"y": state.x + 1}
```

1. Works like `@stanchion_node` but is designed for LangGraph's state-passing pattern.
...
1. `wrap()` finds all nodes decorated with `@stanchion_langgraph_node`, replaces them with validating wrappers, and returns the modified graph. Non-decorated nodes are left untouched.
...
|----------|---------------|
| New pipeline from scratch | Use `StanchionRunner` directly |
...
!!! tip
    The adapter currently provides **contract validation only**. Full runner features (retries, checkpointing, cost tracking) require using `StanchionRunner` directly.

## Recap

- Install with `pip install stanchion[langgraph]`
- Decorate nodes with **`@stanchion_langgraph_node`** to attach contracts
