...
Not all errors are equal. A timeout might resolve on retry; a schema violation never will. Stanchion classifies every exception and applies the right retry strategy automatically.
...
```python
from stanchion import FailureClass, FailurePolicy, StanchionRunner
