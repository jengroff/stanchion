...
### Added
- `StanchionRunner` (replaces `ArmatureRunner`) with decomposed `_execute_node` / `_handle_failure` methods
- `stanchion_node` decorator (replaces `armature_node`)
- `stanchion_langgraph_node` decorator (replaces `armature_langgraph_node`)
...
### Deprecated
- `ArmatureRunner` — use `StanchionRunner` instead (alias retained for backwards compatibility)
- `armature_node` — use `stanchion_node` instead (alias retained)
- `armature_langgraph_node` — use `stanchion_langgraph_node` instead (alias retained)
...
- Redis key prefix changed from `armature:` to `stanchion:`
